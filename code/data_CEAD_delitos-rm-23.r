...
anios_elegidos <- 2023 %>% set_names() # 2005:2024 son todos los disponibles
    ...
    tryCatch({
      ...
      tabla_limpia <- tabla %>%
        ...
        select(nombre_comuna, codigo_comuna, anio, delitos,
               cifra = sym(year_name)) %>%
        mutate(cifra = as.numeric(gsub(",", ".", cifra)))
...
cead_limpiada <- cead_limpiada %>%
  ...
  mutate(delitos = factor(delitos, levels = subgrupo_delitos, labels = subgrupo_delitos),
         cifra = if_else(!is.na(cifra), cifra, NA_real_)) %>%  # Explicitly handle NAs
  # Group by comuna, year, and crime type, then keep only the row with the highest value
  # For example, if Santiago 2023 "robo armado" has values 108 and 83, only 108 is kept
  group_by(nombre_comuna, codigo_comuna, anio, delitos) %>%
  slice_max(cifra, n = 1, with_ties = FALSE) %>%
  ungroup()



# Guardar datos limpios
write_csv(cead_limpiada, "data_processed/delitos/datos_23_tasa_delitos_cada-100000_limpios.csv", na = "")
